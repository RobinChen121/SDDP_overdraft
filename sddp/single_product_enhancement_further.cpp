...
std::array<double, 2> SingleProduct::solve() const {
    ...
    for (int t = 0; t < T + 1; t++) {
        ...
        if (t < T) {
            ...
            if (t == 0)
                models[t].addConstr(iniCash - unit_vari_order_costs[t] * q[t] - W0[t] + W1[t] +
                                            W2[t] ==
                                    overhead_costs[t]);
            else {
                models[t].addConstr(cash[t - 1] - unit_vari_order_costs[t] * q[t] - W0[t] + W1[t] +
                                            W2[t] ==
                                    overhead_costs[t]);
            }
        ...
        if (t == 0)
            models[t].setObjective(overhead_costs[0] + unit_vari_order_costs[0] * q[0] +
    ...
    // remove duplicate cuts during iteration
    ...
    while (iter < iter_num) {
        std::vector results_status(
                T - 1,
                std::vector<std::unordered_map<PairStatus, std::array<std::vector<double>, 2>>>(
                        forward_num));
            ...
            for (int n = 0; n < forward_num; n++) {
                ...
                try {
                    ...
                    models[t].write("iter" + std::to_string(iter + 1) + "_sub_" +
                                    std::to_string(t) + "^" + std::to_string(n + 1) + ".lp");
                    ...
                    if (t < T) {
                        ...
                        if (s > 0) {
                            ...
                            double this_W = this_end_cash - overhead_costs[t] -
                                            unit_vari_order_costs[t] * last_q;
