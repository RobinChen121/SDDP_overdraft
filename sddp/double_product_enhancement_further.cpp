...
std::array<double, 3> DoubleProduct::solve() const {
    ...
    while (iter < iter_num) {
        ...
        std::vector results_status(
                T - 1,
                std::vector<std::unordered_map<TripleStatus, std::array<std::vector<double>, 2>>>(
                        forward_num));
                ...
                if (!cut_coefficients_cache[0].contains(this_coefficients) ||
                    ...
                    models[0].addConstr(
                            ...
                            this_coefficients[0] * iniI + this_coefficients[1] * iniI +
                                    this_coefficients[2] * ((1 + r0) * W0[0] - (1 + r1) * W1[0] -
                                                            (1 + r2) * W2[0]) +
        ...
        for (int t = 1; t < T + 1; t++) {
            ...
            if (iter > 0 && t < T) {
                ...
                for (auto finalCoefficients =
                             remove_duplicate_rows(cut_coefficients); // cutCoefficients
                    ...
                    models[t].addConstr(
                            ...
                            final_coefficient[0] * (I1[t - 1] + q1_pre[t - 1]) +
                                    ...
                                    final_coefficient[2] * ((1 + r0) * W0[t] - (1 + r1) * W1[t] -
                                                            (1 + r2) * W2[t]) +
                    ...
                    if (t < T) {
                        ...
                        if (s > 0) {
                            double this_end_cash =
                                    rhs2 + prices1[t - 1] * rhs1_1 + prices2[t - 1] * rhs1_2;
